...
pub struct CircuitInput {
    ...
    pub min_len: usize, // minimum length in bytes of the input string
}
...
fn compute_var_len_keccak<F: Field>(
    ...
    // We supply the `max_len` and `min_len` as a witnesses as we need to range check it to the size of the input before performing the keccak computation
    ...
    let hash_idx = keccak.keccak_var_len(ctx, range, bytes, Some(inp.bytes31.into_bytes()), max_len, inp.min_len);
